...
public class DateUtils extends org.apache.commons.lang.time.DateUtils {
    ...
    public static final SimpleDateFormat MIDDATEFORMAT = new SimpleDateFormat("yyyyMMddHHmmss");
    ...
    public static String genericTrxNoFromDate(Date date) {
        ...
        String trxNo = SOLONGDATEFORMATE.format(date);//生成前17位
    ...
    public static String getContactNo() {
        ...
        contactNo += SOLONGDATEFORMATE.format(new Date());//生成前17位
    ...
    public static String getMidDateFormat() {
        return MIDDATEFORMAT.format(new Date());
    }
     ...
     * @throws Exception
     * @throws
     * @Title: getCurrMonthFromTrxNo
     * @Description: 通过流水号获取当前月份
     */
    public static int getCurrMonthFromTrxNo(String trxNo) throws Exception {
        if (trxNo == null || trxNo.length() != 27) {
            throw new Exception("通过流水号获取当前月份=>流水号异常");
        }
        String monthStr = trxNo.substring(5, 7);
        Integer currMonth = Integer.parseInt(monthStr);
        return currMonth;
    }
     ...
     * @return
     */
    public static int getCurrMonthFromDate(Date date) {
        if (date == null) {
            date = new Date();
        }
        Calendar calendar = Calendar.getInstance();
     ...
     * @return String    返回类型
     * @Title: getOrderDateFromTrxNo
     * @Description: 根据流水号获取订单日期
     */
    public static String getOrderDateFromTrxNo(String trxNo) throws Exception {
        if (trxNo == null || trxNo.length() != 27) {
            throw new Exception("通过流水号获取订单=>流水号异常");
        }
        String dateString = trxNo.substring(1, 5) + "-" + trxNo.substring(5, 7) + "-" + trxNo.substring(7, 9);
        //String dateString = trxNo.substring(0, 4)+"-"+trxNo.substring(4, 6)+"-"+trxNo.substring(6, 8);//删除，流水号规则已修改
        return dateString;
    }
    ...
    public static List<Date> getBetweenDates(Date start, Date end) {
        ...
        tempStart.add(Calendar.DAY_OF_YEAR, 1);     //起始时间

        Calendar tempEnd = Calendar.getInstance();          //终止时间
        tempEnd.setTime(end);
    ...
    /**
     * 两个日期之间的所有月 yyyy-MM,如果日期月份相等，返回这个月
     *
     * @param minDate
     * @param maxDate
     * @return
     * @throws ParseException
     */
    public static List<String> getBetweenMonth(String minDate, String maxDate,int range) {
        ...
        try {
            ...
            if(range > 0){
                min.add(Calendar.MONTH,(0-range));
                max.add(Calendar.MONTH,range+1);
            }
    ...
    /**
     * 输入日期和日期分隔条件（注意，前包含，后不包含），返回年月：
     * 例子：
     *        输入： date->2019-03-26,day->25  返回 201904
     *        输入： date->2019-03-08,day->8 返回 201903
     *        输入:  date->2019-03-08,day->7 返回201904um
     * @param date 时间
     * @param day 分隔日
     * @return
     */
    public static Integer getAssignYearAngMonth(Date date,int day){
        if(null == date){
            date = new Date();
        }

        int today = Integer.parseInt(DateUtils.formatDate(date,"dd"));
